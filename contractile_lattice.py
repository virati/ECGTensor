...
class c_sync:
    ...
    def get_pos(self):
        ...
        # 3d spring layout
        network_pos=nx.spectral_layout(self.G,dim=3)
    ...
    def run(self):
        ...
        self.tmesh = mlab.triangular_mesh(self.pos[:,0], self.pos[:,1], self.pos[:,2], d2d.vertices,
        ...
        def anim():
            ...
            for ii in np.arange(100):
                ...
                #First, we update the trangulation-manifold
                mplt.set(x=self.pos[:,0],y=self.pos[:,1],z=self.pos[:,2],scalars=self.pos[:,2],triangles=d2d.vertices)
                #Next, we set the node positions
